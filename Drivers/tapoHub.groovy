...
def getAlarmTypes() {
	...
	if (cmdResp != "ERROR") {
		...
		logData << ["data.alarmSounds": "updated"]
	} else {
...
def getAlarmConfig() {
	...
	if (cmdResp != "ERROR") {
		...
		logData << ["attr.alarmConfig": cmdResp]
	} else {
		logData << ["ERROR": "Error in Sysn Comms"]
	}
	return logData
...
//	def alarmData = parseData(resp).cmdResp.result.responses
//	log.warn alarmData
//	def in_alarm = alarmData.find{it.method == "get_device_info"}.result.in_alarm
//	log.warn in_alarm
}
